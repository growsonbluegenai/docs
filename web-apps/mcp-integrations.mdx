...
An MCP server connection gives the build agent tools from an external service, alongside its own.

MCP is the Model Context Protocol, an open standard for exposing a service's capabilities to an AI agent. Connecting one means the agent can act against that service during a build.

## What you'll learn

How to add a server, what the connection states mean, and how credentials are scoped across a project.
