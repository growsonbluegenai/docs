...
Applications built on Veylo are natively hosted by our platform _(with the exception of low code platforms, which may be hosted externally)_. Each application gets a URL you can share, and you can stop and start it from the workspace. 
...
<Columns cols={2}>
  <Column>
    Instead, Users are expected to provide access to their own deployed application or else you may contact Veylo for information about how to get access to a compatible low code environment.
...
Accounts and passwords for the application your are building are handled the Builder agent and are separate and unrelated to your Veylo account credentials and your Project-level collaboration permissions. Instead you will prompt the builder to provide you with access/account credentials suitable to the access that application is using.
...
<CardGroup cols={2}>
  ...
  <Card title="See what the build agent can do" icon="bot" href="/web-apps/build-mode/build-agent-guide">
