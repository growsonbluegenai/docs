...
What to set before you start, what each phase is doing while you wait, and which controls are available once a run is under way.
...
<Steps>
  <Step title="Upload your source material">
    ...
    See [Supported Input Types](/cab/supported-input-types) for what is accepted.
  </Step>
...
Indexing is skipped below 120,000 tokens, a token being roughly a fragment of a word. Small source sets are read whole, so there is nothing to gain from indexing them.
...
When it finishes, the preview fills with the document and the footer reports what the run cost and how long it took. In the example above, a 9 MB source archive generating all 10 sections at **Medium** cost \$6.53 and took 97 minutes 54 seconds.

<Note>
  Cost and elapsed time vary with the volume of source material, the number of sections selected, and the **Model Quality** setting. Treat the figure above as one observed run, not a benchmark.
</Note>
